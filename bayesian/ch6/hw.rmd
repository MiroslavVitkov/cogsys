...
Preparation for the hard section  
```{r}
library(tidyverse)
...
6H0
Fit the following models to the data in d1 ...   
```{r}
...
fit.poly <- function( order=1 )
{
    ...
    if( order > 0 )
    {
        for( i in 1:order )
        {
            second <- paste( second, " + b", i, " * x^", i, "", sep="" )
            third <- paste( third, ", b", i, " ~ dnorm( 0, 10 )", sep="")
        }
    }
    expr <- paste( first, second, third )
    #eval( parse( text=expr ) )

#    eval(parse(text="5+5"))
#    ret <- map( alist( height ~ dnorm( mu , sigma )
#                     , mu <- a + b*weight
#                     , a ~ dnorm( 156 , 100 )
#                     , b ~ dnorm( 0 , 10 )
#                     , sigma ~ dunif( 0 , 50 ) )
...
#m1 <- max.a.posteriori( flist=alist(kcal.per.g ~ dnorm( a , exp(log.sigma) )) 
#                      , data=d1 )
# TODO: how to: import from res/map.r import map as MsdfsdfAsdfsfsPsdsdfs
...
A:p192 p206  
```{r}
# TODO: we are going to neet the R/zWAIC_methods.R here
print("kur")
